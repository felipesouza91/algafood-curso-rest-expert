...
import com.felipe.algafood.api.exceptionhandler.Problem.Field;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler{
	...
	public ResponseEntity<?>  handlerEntidadeNaoEncontrado(EntidadeNaoEncontradaException ex, WebRequest request) {
		HttpStatus status = HttpStatus.NOT_FOUND;
		Problem problem = this.createProblemBuilder(status, ProblemType.RECURSO_NAO_ENCONTRADO, ex.getMessage(), ex.getMessage()).build();
	...
	public ResponseEntity<?> handlerNegocioException(NegocioException ex, WebRequest request) {
		...
		Problem problem = this.createProblemBuilder(status, ProblemType.ERRO_NEGOCIO, ex.getMessage(), ex.getMessage()).build();
		return handleExceptionInternal(ex, problem, new HttpHeaders(), status, request);
	}
	...
	public ResponseEntity<?> handlerEntidadeEmUsoException(EntidadeEmUsoException ex, WebRequest request) {
		...
		Problem problem = this.createProblemBuilder(status, ProblemType.ENTIDADE_EM_USO, ex.getMessage(), ex.getMessage()).build();
		return handleExceptionInternal(ex, problem, new HttpHeaders(), status, request);
	}
	...
	public ResponseEntity<?> handleAccessDeniedException(AccessDeniedException ex, WebRequest request) {
		...
		Problem problem = this.createProblemBuilder(status, ProblemType.ACESSO_NEGADO,
				ex.getMessage(), messageSource.getMessage("acesso-negado", null,Locale.getDefault())).build();
		return this.handleExceptionInternal(ex, problem, new HttpHeaders(), status, request);
	}
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		Problem problem = this.createProblemBuilder(status, ProblemType.MENSAGEM_INCOMPREENSIVEL, detail, MSG_ERRO_GENERICO_USUARIO_FINAL).build();
		return handleExceptionInternal(ex, problem, new HttpHeaders(), status, request);
	}
	...
	protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers,
		...
		if ( ex instanceof MethodArgumentTypeMismatchException) {
			return this.handlerMethodArgumentTypeMismatchException((MethodArgumentTypeMismatchException)ex, headers,status,request);
		}
	...
	protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
		...
		Problem problem = this.createProblemBuilder(status, ProblemType.RECURSO_NAO_ENCONTRADO, detail, MSG_ERRO_GENERICO_USUARIO_FINAL).build();
	...
	private Problem.ProblemBuilder createProblemBuilder( HttpStatus status,
			ProblemType problemType, String detail, String userMessage) {
